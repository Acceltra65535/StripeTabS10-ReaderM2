...
class TerminalViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val readerListener = object : MobileReaderListener {
        ...
        override fun onRequestReaderInput(options: ReaderInputOptions) {
            appendLog("Reader input requested: ${options.name}")
        }

        override fun onRequestReaderDisplayMessage(message: ReaderDisplayMessage) {
            appendLog("Reader message: ${message.name}")
        }
    }
    ...
    fun connectSelectedReader() {
        ...
        currentOperation = Terminal.getInstance().connectReader(
            reader,
    ...
    private fun extractCardLastFour(paymentIntent: PaymentIntent): String? {
        return paymentIntent.charges?.firstOrNull()?.paymentMethodDetails?.cardDetails?.last4
    }
}
